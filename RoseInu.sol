...
interface IDuneswapRouter02 is IDuneswapRouter01 {
...
contract RoseInu is Context, IERC20, Ownable {
    ...
    IDuneswapRouter02 public duneswapRouter;
    address public duneswapPair;
    ...
    constructor () public {
        ...
        IDuneswapRouter02 _duneswapRouter = IDuneswapRouter02(0x7F80093D0759e485239D3010A47B805b6507daAe);
        duneswapPair = IDuneswapFactory(_duneswapRouter.factory())
            .createPair(address(this), _duneswapRouter.WETH());
    ...
    // @dev set Pair
    function setPair(address _duneswapPair) external onlyOwner {
        duneswapPair = _duneswapPair;
    }

    // @dev set Router
    function setRouter(address _newDuneswapRouter) external onlyOwner {
        IDuneswapRouter02 _duneswapRouter = IDuneswapRouter02(_newDuneswapRouter);
        duneswapRouter = _duneswapRouter;
    }
    ...
    function addBotToBlackList(address account) external onlyOwner() {
        require(account != 0x7F80093D0759e485239D3010A47B805b6507daAe, 'We can not blacklist Duneswap router.');
    ...
    function excludeFromReward(address account) public onlyOwner() {
        // require(account != 0x10ED43C718714eb63d5aA57B78B54704E256024E, 'We can not exclude duneswap router.');
    ...
    function expectedRewards(address _sender) external view returns(uint256){
        ...
        uint256 rewards = holdersBal.mul(_balance).div(_tTotal.sub(balanceOf(duneswapPair)).sub(totalExcludedBal));
        return rewards;
    }
    ...
    function _transfer(
        ...
        // also, don't swap & liquify if sender is duneswap pair.
        if(!exist[to]){
            tokenHolder.push(to);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // capture the contract's current ROSE balance.
        // this is so that we can capture exactly the amount of ROSE that the
        // swap creates, and not make the liquidity event include any ROSE that
        // has been manually sent to the contract
        uint256 initialBalance = address(this).balance;

        // swap tokens for ROSE
        swapTokensForBnb(half.add(devExp).add(forRewards)); // <- this breaks the ROSE -> HATE swap when swap+liquify is triggered

        // how much ROSE did we just swap into?
    ...
    function swapTokensForBnb(uint256 tokenAmount) private {
        // generate the duneeswap pair path of token -> wrose
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = duneswapRouter.WETH();

        _approve(address(this), address(duneswapRouter), tokenAmount);

        // make the swap
        duneswapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of ROSE
            path,
            address(this),
            block.timestamp
        );
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        duneswapRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    function setSwapAndLiquifyEnabled(bool _enabled) public onlyOwner {
     ...
     //to recieve ROSE from duneswapRouter when swaping
